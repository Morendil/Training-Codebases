...
public class LegacyGame {
  ...
  public static int WIN_LENGTH = 5;
  ...
  public LegacyGame() {
    second = new Player();
    second.playerMark = MARK_2;

    first = new Player();
    first.playerMark = MARK_1;
  }
  ...
  public int check() {
    if (win(second)) return MARK_2;
    if (win(first)) return MARK_1;
    return (MARK_0);
  }
  ...
  public int checkSeries(int playerMark, int depth) {
    ...
    for (int k = 0; k < TOTAL_SQUARES_PER_BOARD; k++) {
      int stagingDepth = 2;
      if (stagingBoard[k] == EMPTY || gameBoard[stagingDepth][k] != EMPTY) continue;
  ...
  public void respondToMouseUp(int playerMove, int x, int y) {
    ...
    if (check() == ZERO_MARK_FOR_COMPUTER)
      gameState = 3;

    if (check() == X_MARK_FOR_PLAYER)
      gameState = 2;
  ...
  public boolean win(Player p) {
    int index_1;
    int index_2;
    for (index_2 = 0; index_2 < (SQUARES_PER_SIDE - WIN_LENGTH + 1); index_2++) {

      for (index_1 = 0; index_1 < SQUARES_PER_SIDE; index_1++) {
        for (int i = 0; i < 3; i++)
          for (int j = 0; j < 4; j++)
            marksForChecking[i][j] = 0;
        for (int k = 0; k < WIN_LENGTH; k++) {
          incrementWinCountForDirection((index_1 * SQUARES_PER_SIDE
              + index_2 + k), HORIZONTAL);
          incrementWinCountForDirection((index_2 * SQUARES_PER_SIDE
              + index_1 + k * SQUARES_PER_SIDE), VERTICAL);
        }
        if (won(p)) {
          return true;
        }

        if (index_1 < (SQUARES_PER_SIDE - WIN_LENGTH + 1)) {
          for (int i = 0; i < 3; i++)
            for (int j = 0; j < 4; j++)
              marksForChecking[i][j] = 0;
          int winSize = WIN_LENGTH;
          for (int k = 0; k < winSize; k++) {
            incrementWinCountForDirection((index_2 * SQUARES_PER_SIDE
                + index_1 - k * 9 + (winSize - 1) * SQUARES_PER_SIDE),
                DIAGONAL_LEFT);
            incrementWinCountForDirection((index_2 * SQUARES_PER_SIDE
                + index_1 + k * (SQUARES_PER_SIDE + 1)), DIAGONAL_RIGHT);
          }
          if (won(p)) {
            return true;
          }
  ...
  private boolean won(Player p) {
    for (int i = 0; i < 4; i++)
      if (marksForChecking[p.playerMark][i] >= WIN_LENGTH)
        return true;
    return false;
  }
}
